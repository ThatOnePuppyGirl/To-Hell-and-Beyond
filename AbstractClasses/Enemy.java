...
public abstract class Enemy
{
	...
	{
		int maxRNG = (dmgVar * 2) + 1; 				// multiplies dmgVar by 2 then adds 1 because of random generation (allows for negative numbers)
		int dmgVar2 = rng.nextInt(maxRNG) - dmgVar;	// generates random number then adjusts down (to allow for negative numbers)
		int totalDmg = avgDamage + dmgVar2;			// adds dmgVar2 to avgDamage to generate damage
		
		return totalDmg;							// returns totalDmg
	}
	
	// constructors: Name(String name, int index, double health, int level, Item weapon);
}
